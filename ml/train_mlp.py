...
def evaluate(
    ...
    y_val_prob = predict_proba(model, X_val, device)
    y_val_pred = (y_val_prob >= 0.5).astype(int)

    y_test_prob = predict_proba(model, X_test, device)
...
def main() -> None:
    ...
    metrics = evaluate(model, X_val, y_val, X_test, y_test, device)
